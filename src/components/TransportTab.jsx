    ...
    <div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="sns-card">
          {rows.map((r, i) => (
            <div key={r.member.id} className={i > 0 ? 'sns-border-t' : ''} style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
              <div>
              ...
              <div className="flex items-center gap-3">
                {r.unpaidTotal > 0 ? (
                  <>
                    ...
                    <button onClick={() => onMarkPaid(r.member, start, end)} className="sns-btn-primary" style={{ fontSize: '0.78rem', padding: '0.5rem 0.9rem' }}>
                      Mark as Paid
                    </button>
                  </>
                ) : (
